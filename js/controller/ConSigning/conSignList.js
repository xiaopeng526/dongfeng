...
define(function (require, exports, module) {
	...
	function countContSignListNum() {
		var _url = mbank.getApiURL() + 'countContSignListNum.do';
	...
	mui.plusReady(function () {
  	...
  	/*点击选项卡，重新切换数据*/
		...
		var index = 0,
		    pageNo = [{ num: 1, tabLoadEnd: false }, { num: 1, tabLoadEnd: false }, { num: 1, tabLoadEnd: false }];
			...
			loadUpFn: function loadUpFn(me) {
				...
				switch (index) {
					case 0:
						...
						break;
					case 1:
						queryContSignList('01', index, 1, true, searchValue);
						break;
					case 2:
						queryContSignList('02', index, 1, true, searchValue);
						break;
				}
			},
			loadDownFn: function loadDownFn(me) {
				switch (index) {
					case 0:
						...
						break;
					case 1:
						queryContSignList('01', index, pageNo[1].num, false, searchValue);
						break;
					case 2:
						queryContSignList('02', index, pageNo[2].num, false, searchValue);
						break;
				}
			},
			threshold: 50
   ...
   /*按钮点击 ， 跳转页面*/
		...
		$('.table-view').on('tap', '.btn', function () {
			...
			status = $(_this).parents('li').find('.status').attr('data-value'),
			    ...
			    contNo = $(_this).parents('li').find('.applCde').attr('id'); // 合同编号

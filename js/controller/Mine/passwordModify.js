...
define(function (require, exports, module) {
	...
	mui.plusReady(function () {
		...
		userPWDSoftKey.addEventListener("blur", function () {
		...
		userPWDSoftKey.addEventListener("click", function (event) {
			event.preventDefault();
			plus.pluginPGKeyboard.hideKeyboard("pwd");
			plus.pluginPGKeyboard.clearKeyboard("pwd");
			userPWDSoftKey.name = "";userPWDSoftKey.value = "";
			plus.pluginPGKeyboard.openAESKeyboard("pwd", "false", 0, 20, "false", "false", "true", reg, "", passwordSeed, function (result) {
				if (result.status) {
					if (plus.os.name == 'iOS') {
						...
						userPWDSoftKey.value = obj.cipherText == null ? "" : obj.text;
						userPWDSoftKey.name = obj.cipherText == null ? "" : obj.cipherText;
					} else {
		...
		submit.addEventListener('tap', function () {
			...
			if (userPWDSoftKey.value == "") {
				mCheck.toast("原密码不能为空");
				return false;
			}
			...
			if (userPWDSoftKey.name == userPWDSoftKey2.name) {
				mCheck.toast("新密码和原密码不能相同");
				return false;
			}
			...
			var regData = {
				"session_customerId": loginId,
				"Khmm": userPWDSoftKey.name,
				"confirmPassword": userPWDSoftKey2.name,
				"passwdSeed": passwordSeed
			};
			var url = mBank.getApiURL() + 'passwordModiFication.do';
			mBank.apiSend('post', url, regData, function (data) {
				mCheck.alert('密码设置成功,请重新登录', function () {

...
(function($, window, document) {
	...
	var GroupList = $.GroupList = $.Class.extend({
		...
		caleLayout: function() {
			var self = this;
//			var withoutSearchHeight = (self.box.offsetHeight - self.el.search.offsetHeight-24) + 'px';
//			self.el.bar.style.height = withoutSearchHeight;
//			self.el.inner.style.height = withoutSearchHeight;
//			var barItemHeight = ((self.el.bar.offsetHeight - 64) / self.el.barItems.length) + 'px';
...
//			});
		...
		bindBarEvent: function() {
			...
			var findStart = function(event) {
				...
				pointElement = document.elementFromPoint(point.pageX, point.clientY);
				if (pointElement) {
